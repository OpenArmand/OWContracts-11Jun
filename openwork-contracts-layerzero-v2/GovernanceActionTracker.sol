    ...
    IBridgeMessageReceiver
{
    ...
    mapping(uint32 => address) public chainIdToTrackerAddress;
    ...
    event ChainTrackerAddressSet(uint32 chainId, address trackerAddress);
     ...
     */
    function setChainTrackerAddress(uint32 chainId, address trackerAddress) external onlyOwner {
     ...
     */
    function sendCrossChainGovernanceAction(uint32 chainId, address user) external returns (bool success) {
        // Only owner, this contract, or authorized contracts can record governance actions
        require(
            msg.sender == owner() || 
            msg.sender == address(this) || 
