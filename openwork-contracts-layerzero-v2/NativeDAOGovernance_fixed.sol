    ...
    IBridgeMessageReceiver
{
     ...
     */
    function setMainChainId(uint64 _mainChainId) external onlyOwner {
    ...
    function receiveCrossChainMessage(
        ...
        if (messageType == 1) {
            ...
            // Just decode but don't use variables that will be implemented later
            abi.decode(data, (address, bool));
            return true;
        } else if (messageType == 2) {
            // Process governance vote
            // Just decode but don't use variables that will be implemented later
            abi.decode(data, (uint256, address, uint8));
            return true;
        }
